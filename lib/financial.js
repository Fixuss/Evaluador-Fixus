...
export function calcularRatios(data) {
  const {
    ...
    act_co, act_nco, pas_co, pas_nco, pn, caja,
    dcp, dlp, ventas_ant, meses_post
  } = data

  const gan_bruta   = ventas - cmv
  const ebitda      = gan_bruta - gastos_op + amort
  const ebit        = ebitda - amort
  const res_neto    = ebit + res_fin - imp
  const activo_total  = act_co + act_nco
  const pasivo_total  = pas_co + pas_nco
  const deuda_fin   = dcp + dlp
  const intereses   = Math.abs(res_fin)
  ...
  return {
    gan_bruta, ebitda, ebit, res_neto,
    activo_total, pasivo_total, deuda_fin, intereses, capital_trabajo,
    margen_bruto:  ventas > 0 ? gan_bruta / ventas * 100 : 0,
    margen_ebitda: ventas > 0 ? ebitda / ventas * 100 : 0,
    margen_neto:   ventas > 0 ? res_neto / ventas * 100 : 0,
    liquidez:      pas_co > 0 ? act_co / pas_co : 0,
    endeudamiento: pn > 0 ? pasivo_total / pn : 0,
    cobertura:     intereses > 0 ? ebitda / intereses : 99,
    roe: pn > 0 ? res_neto / pn * 100 : 0,
    roa: activo_total > 0 ? res_neto / activo_total * 100 : 0,
...
export function evalElegibilidad(r, antiguedad, fin_sol, F) {
  const checks = [
    {
    ...
    },
    {
      ...
      label: 'Financiamiento dentro del límite',
