...
export function buildResena(p, pipelineMatch = null) {
  ...
  if (ok(p.facturacion_aprox)) dim.push(`una facturación aproximada de $${p.facturacion_aprox.trim()}`)
  ...
  // ── 2. Historia y origen ────────────────────────────────────────────────────
  if (ok(p.historia)) {
    secciones.push({ titulo: '2. Historia y origen', texto: p.historia.trim() })
  }
  ...
  const partes4 = []
  if (ok(p.negocio)) partes4.push(p.negocio.trim())
  ...
  if (cliArr.length > 0) {
    const listaCli = cliArr.map(c => {
      const det = [
        ok(c.porcentaje) ? `representa el ${c.porcentaje.trim()} de la facturación` : null,
  ...
  if (provArr.length > 0) {
    const listaProv = provArr.map(pr => {
      const det = [
        ok(pr.porcentaje) ? `representa el ${pr.porcentaje.trim()} de las compras` : null,
  ...
  const partes6 = []
  if (ok(p.infraestructura)) partes6.push(p.infraestructura.trim())
