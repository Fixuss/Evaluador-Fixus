...
function EstadoFinal({ icon, title, text, color='#059669' }) {
  return (
  ...
  return (
    <>
      ...
      <div style={{ maxWidth:780, margin:'0 auto', padding:'28px 16px 60px' }}>
        ...
        {status === 'notfound' && <EstadoFinal icon="🔍" title="Formulario no encontrado" color='#DC2626' text="El link no es válido o fue eliminado. Pedile un nuevo link al analista de Fixus." />}
        {status === 'error' && <EstadoFinal icon="⚠️" title="Error al cargar" color='#D97706' text="Ocurrió un error inesperado. Intentá recargar la página." />}
        {status === 'already_submitted' && <EstadoFinal icon="✅" title="Formulario ya enviado" color='#059669' text={`Este formulario ya fue completado. El equipo de Fixus tiene la información. ¡Gracias${razon ? `, ${razon}` : ''}!`} />}
        {status === 'submitted' && <EstadoFinal icon="🎉" title="¡Formulario enviado!" color='#059669' text="Tu información fue recibida. El equipo de Fixus se pondrá en contacto a la brevedad. ¡Muchas gracias!" />}
        ...
        {(status === 'ready' || status === 'submitting') && form && (
          <>
            <div style={{ background:'#fff', border:'1px solid rgba(74,105,204,.18)', borderRadius:12, padding:'16px 20px', marginBottom:24, fontSize:13, color:'#334155', lineHeight:1.6 }}>
              <strong style={{ color:'#0e2c50' }}>Instrucciones:</strong> Completá los campos de cada sección. Al finalizar hacé clic en <strong>"Enviar formulario"</strong>.
            </div>
              ...
              <div key={si} className="card section-gap card--indigo">
                ...
                <div className="card-body">
                  <div className="form-grid">
                    {sec.campos.map(c => <Campo key={c.id} def={c} form={form} setForm={setForm} />)}
                  </div>
                </div>
              </div>
            ))}
            ...
            <div style={{ marginTop:28, display:'flex', alignItems:'center', gap:14 }}>
              <button className="btn btn-success" onClick={handleSubmit} disabled={status === 'submitting'} style={{ fontSize:15, padding:'13px 32px', fontWeight:700 }}>
                {status === 'submitting' ? <><span className="spinner" /> Enviando…</> : '✓ Enviar formulario'}
              </button>
              <div style={{ fontSize:12, color:'#94a3b8' }}>Datos enviados de forma segura a Fixus</div>
            </div>
          </>
        )}
      </div>
